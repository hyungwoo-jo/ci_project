...
run_one_simulation <- function(sim_id, n, true_ate) {
  ...
  Y0 <- 5*X1 + 2*(X1^2) - 3*X2 + X2*X3 + 2*X4 + 4*U + rnorm(n, 0, 2)
  Y1 <- Y0 + true_ate
  Y <- Y1 * T + Y0 * (1 - T)
...
sink()
cat("
Summary results saved to simulation_summary_results_uc.txt
")
...
p <- ggplot(final_results_df, aes(x = fct_reorder(Method, Estimated_ATE, .fun=median, .desc=FALSE, .na_rm = TRUE), y = Estimated_ATE, fill = Method)) +
  ...
  labs(title = "Distribution of ATE Estimates with Unknown Confounder",
       x = "Methodology",
...
ggsave("simulation_results_boxplot_uc.png", plot = p, width = 12, height = 8, dpi = 300)

cat("
Boxplot graph saved to simulation_results_boxplot_uc.png
")
